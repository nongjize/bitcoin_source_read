...
CBlockIndex * InsertBlockIndex(uint256 hash)
{
    ...
    mi = mapBlockIndex.insert(make_pair(hash, pindexNew)).first;//insert() 方法返回一对pair对象（迭代器，bool）
    pindexNew->phashBlock = &((*mi).first);
...
{
    if (!pblocktree->LoadBlockIndexGuts())//用 CBlockTreeDB类的全局变量实例 pblocktree 从数据库读取区块 并验证 写入全局变量mapBlockIndex
        return false;

    boost::this_thread::interruption_point();//预埋一个打断点
    ...
    try {
        ...
        while (blkdat.good() && !blkdat.eof()) {
            ...
            try {
                ...
                LogPrintf("%s : Deserialize or I/O error - %s", __FUNCTION__, e.what());
            }
        }
        fclose(fileIn);
    } catch(std::runtime_error &e) {
...
#pragma region 处理MSG_BLOCK和MSG_FILTERED_BLOCK
                bool send = false;
