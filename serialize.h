...
#pragma region 定义宏IMPLEMENT_SERIALIZE
...
#pragma region GetSerializeSize
...
uint64_t ReadCompactSize(Stream& is)
{
    ...
    else
    {
        ...
        READDATA(is, xSize);
        nSizeRet = xSize;
        if (nSizeRet < (unsigned __int64)0x100000000)
            throw std::ios_base::failure("non-canonical ReadCompactSize()");
    }
...
public:
    ...
    CDataStream(const std::vector<unsigned char>& vchIn, int nTypeIn, int nVersionIn) : vch((char*)&vchIn.begin(), (char*)&vchIn.end())
    {
        Init(nTypeIn, nVersionIn);
    }
