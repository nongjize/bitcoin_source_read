...
void StartRPCThreads()
{
    ...
    {
#pragma region 如果-rpcuser和-rpcpassword不对，提示并start shutdown.
...
#pragma endregion
    ...
    if (fUseSSL)
    {
#pragma region 配置sll::context对象rpc_ssl_context
...
#pragma endregion
    ...
    try
    {
#pragma region 创建一个 ip::tcp::acceptor的智能指针，配置并开始Listen
...
void StopRPCThreads()
{
    ...
    BOOST_FOREACH(const boost::shared_ptr<ip::tcp::acceptor> &acceptor, rpc_acceptors)
    {
        acceptor->cancel(ec);
        if (ec)
            LogPrintf("%s: Warning: %s when cancelling acceptor", __FUNCTION__, ec.message());
    }
    rpc_acceptors.clear();
    BOOST_FOREACH(const PAIRTYPE(std::string, boost::shared_ptr<deadline_timer>) &timer, deadlineTimers)
    {
        ...
        if (ec)
            LogPrintf("%s: Warning: %s when cancelling timer", __FUNCTION__, ec.message());
    }
