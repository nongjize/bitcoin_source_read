...
bool EvalScript(vector<vector<unsigned char> >& stack, const CScript& script, const CTransaction& txTo, unsigned int nIn, unsigned int flags, int nHashType)
{
    ...
    try //try 走完脚本
    {
        while (pc < pend)//逐个计算运行脚本
        {
#pragma region //while
            bool fExec = !count(vfExec.begin(), vfExec.end(), false);//vfexec中只要有一个false，fexec即为false,当没有false时为true
            
            if (!script.GetOp(pc, opcode, vchPushValue))// Read instruction
                return false;
...
#pragma endregion		
                ...
                return false; // Disabled opcodes//禁用操作符.

            if (fExec && 0 <= opcode && opcode <= OP_PUSHDATA4)         // 只push入数据//
                stack.push_back(vchPushValue);                          //              // 如果有if嵌套，用于记录当前位置是否执行 
            else if (fExec || (OP_IF <= opcode && opcode <= OP_ENDIF))  //if位置进行执行 或者 opcode为 if-endif 之间的操作符
				switch (opcode)
			{
...
#pragma region //case
                    ...
                    bool fSuccess = IsCanonicalSignature(vchSig, flags) //check if is canonical 标准范式
						         && IsCanonicalPubKey(vchPubKey, flags) //check if is canonical 标准范式
