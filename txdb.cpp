...
{
    leveldb::Iterator *pcursor = NewIterator(); //直接读取levedb中的数据 通过leveldb的迭代器
    ...
    while (pcursor->Valid()) 
	{
		...
		{
#pragma region
            leveldb::Slice slKey = pcursor->key();//获取键
            CDataStream ssKey(slKey.data(), slKey.data()+slKey.size(), SER_DISK, CLIENT_VERSION);
            char chType;
            ssKey >> chType;
            if (chType == 'b') 
			{
                ...
                ssValue >> diskindex;//反序列化操作 从数据库中读取数据 写入 CDiskBlockIndex 实列。

                // Construct block index object
                CBlockIndex* pindexNew    = InsertBlockIndex(diskindex.GetBlockHash());//插入到 全局变量 mapBlockIndex
